class TailwindFormBuilder < ActionView::Helpers::FormBuilder
  ...
  def email_input(name, *args, &block)
    input_element(:email_field, name, *args, &block)
  end

  def password_input(name, *args, &block)
    input_element(:password_field, name, *args, &block)
  end

  def number_input(name, *args, &block)
    input_element(:number_field, name, *args, &block)
  end

  def hidden_input(name, *args, &block)
    hidden_field(name, *args, &block)
  end

  def date_input(name, *args, &block)
    # input_element(:date_field, name, *args, &block)
    datetime_element(:date_field, name, *args, &block)
  end

  def time_input(name, *args, &block)
    datetime_element(:time_field, name, *args, &block)
  end
  ...
  def radio_button(name, *args)
    ...
    @template.content_tag(:div, { class: 'form-element pt-4', id: "field-#{name}" }) do
      @template.concat(@template.content_tag(:div, { class: 'flex items-center' }) do
        @template.concat(radio_button(name,
                                      { class: 'focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300' + (error ? ' invalid-input' : '') }.merge(options)))
    ...
    @template.content_tag(:div, { class: 'form-element pt-4', id: "field-#{name}" }) do
      ...
      @template.concat(@template.content_tag(:div, { class: 'relative mt-1' }) do
        ...
        @template.concat(send(field_type, name,
                              { class: 'shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md' + (block.present? ? ' pl-10' : '') + (error ? ' invalid-input' : '') }.merge(options)))
      end)
      @template.concat(hint_message(message, error))
    end
  end
